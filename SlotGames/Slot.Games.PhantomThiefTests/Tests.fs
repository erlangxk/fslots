...
[<Fact>]
let ``test count gems 1, multiple gems on same reel, but count as 1`` () =
    let ss2 =[| [| 2; 4; 7 |]; [| 3; 3; 3 |]; [| 9; 4; 0 |]; [| 4; 9; 2 |]; [| 3; 4; 1 |] |]
    let r = Common.countGems [ 3 ] ss2
    let er = Map [ 3, [ (1, 0); (4, 0) ] ]
    Assert.Equal<Map<int, list<int * int>>>(er, r)
    
[<Fact>]
let ``test count gems 2, multiple gems on same reel, but count as 1`` () =
    let ss2 =[| [| 2; 4; 7 |]; [| 3; 3; 3 |]; [| 9; 4; 0 |]; [| 4; 9; 2 |]; [| 3; 3; 1 |] |]
    let r = Common.countGems [ 3 ] ss2
    let er = Map [ 3, [ (1, 0); (4, 0) ] ]
    Assert.Equal<Map<int, list<int * int>>>(er, r)
    
[<Fact>]
let ``test count gems 3, multiple gems on same reel, but count as 1`` () =
    let ss2 =[| [| 2; 4; 7 |]; [| 3; 2; 3 |]; [| 9; 4; 0 |]; [| 4; 9; 2 |]; [| 3; 3; 1 |] |]
    let r = Common.countGems [ 3 ] ss2
    let er = Map [ 3, [ (1, 0); (4, 0) ] ]
    Assert.Equal<Map<int, list<int * int>>>(er, r)
    
[<Fact>]
let ``test count gems 4, multiple gems on same reel, but count as 1`` () =
    let ss2 =[| [| 2; 4; 7 |]; [| 2; 3; 3 |]; [| 9; 4; 0 |]; [| 4; 9; 2 |]; [| 1; 3; 3 |] |]
    let r = Common.countGems [ 3 ] ss2
    let er = Map [ 3, [ (1, 1); (4, 1) ] ]
    Assert.Equal<Map<int, list<int * int>>>(er, r)

[<Fact>]
let ``count gems 1`` () =
    let r = Common.countGems [ 3; 4 ] snapshot
    let er = Map [ 3, [ (1, 0); (4, 0) ]; 4, [ (0, 1); (2, 1); (3, 0); (4, 1) ] ]
    Assert.Equal<Map<int, list<int * int>>>(er, r)


[<Fact>]
let ``count gems 2`` () =
    let r = Common.countGems [ 2; 7 ] snapshot
    let er = Map [ 2, [ (0, 0); (1, 2); (3, 2) ]; 7, [ (0, 2) ] ]
    Assert.Equal<Map<int, list<int * int>>>(er, r)
...
let ``calc gems multiplier`` () =
    ...
    let (tm, rs) = Common.countGemsResult [ 3; 4 ] payTable snapshot
    let ers = [ 4, [ (0, 1); (2, 1); (3, 0); (4, 1) ], 20; 3, [ (1, 0); (4, 0) ], 2 ]
    Assert.Equal(22, tm)
    Assert.Equal<Common.GemWinResult<int>>(ers, rs)

    let (tm, rs) = Common.countGemsResult [ 2; 7 ] payTable snapshot
    let ers = []
    Assert.Equal(0, tm)
    Assert.Equal<Common.GemWinResult<int>>(ers, rs)
...
let fullCycleMainGame game reels lens =
    ...
    let folder state result =
        ...
        (totalFsOfSpin + freeSpin1,
         totalSpinMul + lineMul,
         totalSpin + 1,
         totalFsOfCollapse + freeSpin2,
         totalCollapseMul + collapseMul,
...
let fullCycleFeatureGame game reels lens =
    ...
    let folder state result =
        let (totalFsOfSpin,
             ...
             totalSpin,
             totalFsOfCollapse,
             totalCollapseLineMul,
             totalCollapseGemsMul,
             totalCollapse,
             collapseDict) =
            state

        let spinFs, spinLineMul, spinGemsMul, collapseFs, collapseLineMul, collapseGemsMul, collapseCount =
            result

        (totalFsOfSpin + spinFs,
         totalSpinLineMul + spinLineMul,
         totalSpinGemsMul + spinGemsMul,
         totalSpin + 1,
         totalFsOfCollapse + collapseFs,
         totalCollapseLineMul + collapseLineMul,
         totalCollapseGemsMul + collapseGemsMul,
         totalCollapse + collapseCount,
         Map.change
             collapseCount
             (fun x ->
                 match x with
    ...
    let totalCollapseGemsWin: int64 =
        int64 totalCollapseGemsMul * int64 Config.Line.totalLines

    let totalWin: int64 =
        (int64 totalCollapseLineMul)
        + (int64 totalSpinLineMul)
        + totalSpinGemsWin
        + totalCollapseGemsWin
...
let ``test RTP with game state switch `` () =
    ...
    while totalSpin < max do
        ...
        else if (s.name = MainGame.MainGameA || s.name = MainGame.MainGameB) then
    ...
    let totalWin =
        totalMainSpinMul
        + totalMainCollapseMul
        + totalFeatureSpinMul
        + totalFeatureCollapseMul
